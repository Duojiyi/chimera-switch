...
### 🌐 唯一官方网站：**[api.chimerahub.org](https://api.chimerahub.org)**
...
**Chimera Switch** 为你提供一个桌面应用来管理所有支持的 AI 工具。无需手动编辑配置文件，你将获得一个可视化界面，一键将供应商导入应用，一键在不同的供应商之间进行切换，内置 ChimeraHub 中转站与官方登录预设、统一的 MCP, SKILLS 管理以及系统托盘即时切换功能——所有操作都基于可靠的 SQLite 数据库和原子写入机制，保护你的配置不被损坏。

- **一个应用，八个工具** — 在单一界面中管理 Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw 和 Hermes
- **告别手动编辑** — 内置 ChimeraHub 中转站预设与各官方登录入口；一键即可切换
...
- **8 个支持工具，精选预设** — Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes；复制 key 即可一键导入
...
- **Deep Link** (`chimeraswitch://`) — 通过 URL 一键导入供应商、MCP 服务器、提示词和技能
- 深色 / 浅色 / 跟随系统主题、开机自启、自动更新、原子写入、自动备份、国际化（简中/繁中/英/日）

## 常见问题

<details>
<summary><strong>Chimera Switch 支持哪些 AI 工具？</strong></summary>

Chimera Switch 支持八个工具：**Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**Grok Build**、**OpenCode**、**OpenClaw** 和 **Hermes**。每个工具都有专属的供应商预设和配置管理。
...
Chimera Switch 使用“通用配置片段”功能，在不同的供应商之间传递 Key 和请求地址之外的通用数据，您可以在“编辑供应商”菜单的“通用配置面板”里，点击“从当前供应商提取”，把所有的通用数据提取到通用配置中，之后在新建“供应商”的时候，只要勾选“应用通用配置”（默认勾选），就会把插件等数据写入到新的供应商配置中。您的所有配置项都会保存在运行本软件的时候，第一次导入的默认供应商里面，不会丢失。
...
Chimera Switch macOS 版本已通过 Apple 代码签名和公证，可直接下载安装，无需额外操作。推荐使用 `.dmg` 安装包。
...
- **数据库**：`~/.chimera-switch/chimera-switch.db`（SQLite — 供应商、MCP、提示词、技能）
- **本地设置**：`~/.chimera-switch/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.chimera-switch/backups/`（自动轮换，保留最近 10 个）
- **SKILLS**：`~/.chimera-switch/skills/`（默认通过软链接连接到对应应用）
- **技能备份**：`~/.chimera-switch/skill-backups/`（卸载前自动创建，保留最近 20 个）
...
从 [Releases](../../releases) 页面下载最新版本的 `Chimera-Switch-v{版本号}-Windows.msi` 安装包或 `Chimera-Switch-v{版本号}-Windows-Portable.zip` 绿色版。
...
从 [Releases](../../releases) 页面下载 `Chimera-Switch-v{版本号}-macOS.dmg`（推荐）或 `.zip`。

> **注意**：Chimera Switch macOS 版本已通过 Apple 代码签名和公证，可直接安装打开。
...
- `Chimera-Switch-v{版本号}-Linux.deb`（Debian/Ubuntu）
- `Chimera-Switch-v{版本号}-Linux.rpm`（Fedora/RHEL/openSUSE）
- `Chimera-Switch-v{版本号}-Linux.AppImage`（通用）
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.chimera-switch/chimera-switch.db`（SQLite）
