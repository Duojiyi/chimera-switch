...
/**
 * Hermes Agent configuration API (Chimera Switch side).
 *
 * Chimera Switch intentionally keeps its Hermes surface minimal — deep configuration
 * (model, agent behavior, env vars, skills, cron, logs, analytics) lives in
 * the Hermes Web UI at http://127.0.0.1:9119. Chimera Switch only reads the `model`
