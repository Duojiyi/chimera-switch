...
[![Version](https://img.shields.io/github/v/release/Duojiyi/chimera-switch?color=blue&label=version)](https://github.com/Duojiyi/chimera-switch/releases)
[![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-lightgrey.svg)](https://github.com/Duojiyi/chimera-switch/releases)
...
### 🌐 Die einzige offizielle Website: **[api.chimerahub.org](https://api.chimerahub.org)**
...
**Chimera Switch** gibt Ihnen eine einzige Desktop-App, um alle unterstützten KI-Werkzeuge zu verwalten. Statt Konfigurationsdateien von Hand zu bearbeiten, erhalten Sie eine visuelle Oberfläche, um Anbieter mit einem Klick zu importieren und sofort zwischen ihnen zu wechseln — mit integriertem ChimeraHub-Relay-Preset plus offiziellen Login-Presets, einheitlicher MCP- und Skills-Verwaltung und schnellem Umschalten über das System-Tray. Das Ganze gestützt auf eine zuverlässige SQLite-Datenbank mit atomaren Schreibvorgängen, die Ihre Konfigurationen vor Beschädigung schützen.
...
- **Kein manuelles Bearbeiten mehr** — integriertes ChimeraHub-Relay-Preset plus offizielle Login-Einträge; einfach auswählen und umschalten
...
- **8 unterstützte Werkzeuge, kuratierte Presets** — Claude Code, Claude Desktop, Codex, Gemini CLI, Grok Build, OpenCode, OpenClaw, Hermes; Schlüssel kopieren und mit einem Klick importieren
...
- **Deep Link** (`chimeraswitch://`) — Importieren Sie Anbieter, MCP-Server, Prompts und Skills per URL
...
<details>
<summary><strong>Welche KI-Werkzeuge unterstützt Chimera Switch?</strong></summary>

Chimera Switch unterstützt acht Werkzeuge: **Claude Code**, **Claude Desktop**, **Codex**, **Gemini CLI**, **Grok Build**, **OpenCode**, **OpenClaw** und **Hermes**. Jedes Werkzeug verfügt über dedizierte Anbieter-Presets und Konfigurationsverwaltung.
...
Chimera Switch für macOS ist von Apple code-signiert und notarisiert. Sie können es direkt herunterladen und installieren — es sind keine zusätzlichen Schritte erforderlich. Wir empfehlen die Verwendung des `.dmg`-Installationsprogramms.
...
- **Datenbank**: `~/.chimera-switch/chimera-switch.db` (SQLite — Anbieter, MCP, Prompts, Skills)
- **Lokale Einstellungen**: `~/.chimera-switch/settings.json` (gerätebezogene UI-Einstellungen)
- **Backups**: `~/.chimera-switch/backups/` (automatisch rotiert, behält die 10 neuesten)
- **Skills**: `~/.chimera-switch/skills/` (standardmäßig per Symlink mit den entsprechenden Apps verbunden)
- **Skill-Backups**: `~/.chimera-switch/skill-backups/` (vor der Deinstallation automatisch erstellt, behält die 20 neuesten)
...
```bash
CC_SWITCH_GDK_BACKEND=wayland ./Chimera-Switch-*.AppImage
```
...
Laden Sie das neueste Installationsprogramm `Chimera-Switch-v{version}-Windows.msi` oder die portable Version `Chimera-Switch-v{version}-Windows-Portable.zip` von der Seite [Releases](../../releases) herunter.
...
```bash
brew install --cask chimera-switch
```

Aktualisieren:

```bash
brew upgrade --cask chimera-switch
```

**Methode 2: Manueller Download**

Laden Sie `Chimera-Switch-v{version}-macOS.dmg` (empfohlen) oder `.zip` von der Seite [Releases](../../releases) herunter.

> **Hinweis**: Chimera Switch für macOS ist von Apple code-signiert und notarisiert. Sie können es direkt installieren und öffnen.
...
- `Chimera-Switch-v{version}-Linux.deb` (Debian/Ubuntu)
- `Chimera-Switch-v{version}-Linux.rpm` (Fedora/RHEL/openSUSE)
- `Chimera-Switch-v{version}-Linux.AppImage` (universell)
...
- **SSOT** (Single Source of Truth): Alle Daten werden in `~/.chimera-switch/chimera-switch.db` (SQLite) gespeichert
...
[![Star History Chart](https://api.star-history.com/svg?repos=Duojiyi/chimera-switch&type=Date)](https://www.star-history.com/#Duojiyi/chimera-switch&Date)
