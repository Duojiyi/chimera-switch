...
This directory contains the Flatpak manifest (`com.chimerahub.switch`) for Chimera Switch, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/chimera-switch.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.chimerahub.switch.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo Chimera-Switch-Linux.flatpak com.chimerahub.switch
```

4) Install and run:

```bash
flatpak install --user ./Chimera-Switch-Linux.flatpak
flatpak run com.chimerahub.switch
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.chimerahub.switch.yml` with more precise grants:

```yaml
  - --filesystem=~/.chimera-switch:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.chimerahub.switch.yml` accordingly.
