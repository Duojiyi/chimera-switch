...
### 🌐 The Only Official Website: **[api.chimerahub.org](https://api.chimerahub.org)**
...
**Chimera Switch** gives you a single desktop app to manage all supported AI tools. Instead of editing config files by hand, you get a visual interface to import providers with one click, switch between them instantly, with the built-in ChimeraHub relay preset plus official login entries, unified MCP and Skills management, and system tray quick switching — all backed by a reliable SQLite database with atomic writes that protect your configs from corruption.
...
- **No More Manual Editing** — built-in ChimeraHub relay preset plus official login entries; just pick and switch
...
- **8 supported tools, curated presets** — Claude Code, Claude Desktop, Codex, Gemini CLI, Grok Build, OpenCode, OpenClaw, Hermes; copy your key and import with one click
...
- **Deep Link** (`chimeraswitch://`) — Import providers, MCP servers, prompts, and skills via URL
- Dark / Light / System theme, auto-launch, auto-updater, atomic writes, auto-backups, i18n (zh/zh-TW/en/ja)

## FAQ

<details>
<summary><strong>Which AI tools does Chimera Switch support?</strong></summary>

Chimera Switch supports eight tools: **Claude Code**, **Claude Desktop**, **Codex**, **Gemini CLI**, **Grok Build**, **OpenCode**, **OpenClaw**, and **Hermes**. Each tool has dedicated provider presets and configuration management.
...
Chimera Switch for macOS is code-signed and notarized by Apple. You can download and install it directly — no extra steps needed. We recommend using the `.dmg` installer.
...
Chimera Switch follows a "minimal intrusion" design principle — even if you uninstall the app, your CLI tools will continue to work normally. The system always keeps one active configuration, because deleting all configurations would make the corresponding CLI tool unusable. If you rarely use a specific CLI tool, you can hide it in Settings. To switch back to official login, see the next question.
...
- **Database**: `~/.chimera-switch/chimera-switch.db` (SQLite — providers, MCP, prompts, skills)
- **Local settings**: `~/.chimera-switch/settings.json` (device-level UI preferences)
- **Backups**: `~/.chimera-switch/backups/` (auto-rotated, keeps 10 most recent)
- **Skills**: `~/.chimera-switch/skills/` (symlinked to corresponding apps by default)
- **Skill Backups**: `~/.chimera-switch/skill-backups/` (created automatically before uninstall, keeps 20 most recent)
...
Download the latest `Chimera-Switch-v{version}-Windows.msi` installer or `Chimera-Switch-v{version}-Windows-Portable.zip` portable version from the [Releases](../../releases) page.
...
Download `Chimera-Switch-v{version}-macOS.dmg` (recommended) or `.zip` from the [Releases](../../releases) page.

> **Note**: Chimera Switch for macOS is code-signed and notarized by Apple. You can install and open it directly.
...
- `Chimera-Switch-v{version}-Linux.AppImage` (Universal)
...
- **SSOT** (Single Source of Truth): All data stored in `~/.chimera-switch/chimera-switch.db` (SQLite)
