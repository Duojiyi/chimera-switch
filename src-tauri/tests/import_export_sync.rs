...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = chimera_switch_lib::get_codex_auth_path();
    let config_path = chimera_switch_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    chimera_switch_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(chimera_switch_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = chimera_switch_lib::get_codex_config_path();
    ...
    chimera_switch_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = chimera_switch_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        chimera_switch_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        chimera_switch_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    let err = chimera_switch_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        chimera_switch_lib::AppError::Config(msg) => {
    ...
    assert!(
        !chimera_switch_lib::get_codex_config_path().exists(),
...
"#;
    ...
    chimera_switch_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        chimera_switch_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = chimera_switch_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        chimera_switch_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        chimera_switch_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = chimera_switch_lib::import_from_codex(&mut config).expect("import codex");
...
"#,
    ...
    config.mcp.servers.as_mut().unwrap().insert(
        ...
        chimera_switch_lib::McpServer {
            ...
            apps: chimera_switch_lib::McpApps {
                claude: false,
                codex: false, // 初始未启用
                gemini: false,
                grokbuild: false,
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    chimera_switch_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = chimera_switch_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = chimera_switch_lib::import_from_claude(&mut config).expect("import from claude");
