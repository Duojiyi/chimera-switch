...
/// provider is routed through Chimera Switch.  A dedicated id is an ownership
...
pub(crate) fn codex_api_key_auth_value(key: &str) -> Value {
    json!({
        "auth_mode": "apikey",
        "OPENAI_API_KEY": key,
    })
...
pub fn apply_codex_official_proxy_route(
    ...
    // Clean only Chimera Switch's placeholder from every stale provider table. Real
...
/// Whether a live Codex config is the official route projected by Chimera Switch.
...
/// Remove only the official takeover route owned by Chimera Switch. This is a
...
"#;
    ...
    fn set_catalog_json_some_preserves_user_owned_catalog() {
        // When Chimera Switch generates a catalog (Some arm), it must still respect a
