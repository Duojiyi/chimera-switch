...
// 6) 断言赞助商预设的上游测试文件不存在
// ---------------------------------------------------------------------------
// 这些测试断言被剪掉的第三方预设仍然存在，留下就会挡住同步门禁。debrand 负责删除，
// 这里断言删除已生效——上游无冲突地重新加回同名文件时也能被抓住。
for (const relPath of CONFIG.sponsorTestFiles?.files ?? []) {
  if (exists(relPath)) {
    fail(`${relPath}: 断言赞助商预设的上游测试文件应已删除（重跑 node scripts/debrand.mjs）`);
  }
}
...
console.log(
  "[check-debrand] 通过：预设、i18n、README、scheme、身份字段、赞助商测试文件均干净。",
);
